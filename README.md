...
PDF conversion needs the **Node.js** runtime (not Edge). Rendering uses [pdf.js](https://github.com/mozilla/pdf.js) via `pdf-to-img`, which requires **`@napi-rs/canvas`** on the server (it is not bundled inside those packages). The function uses `maxDuration: 60`; on the Hobby plan the limit is lower—use Pro or reduce PDF size/page count if you hit timeouts.
